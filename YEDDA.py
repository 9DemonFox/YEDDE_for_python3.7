...
class Example(Frame):
    def __init__(self, parent):
        ...
        # TODO 修改config
        self.configFile = "configs/talent_radar.config"
        self.entityRe = r'\[\@.*?\#.*?\*\](?!\#)'
    ...
    def deleteTextInput(self,event):
        ...
        if len(get_input) > 0:
            followHalf_content = followHalf_content.replace(get_input, ''.encode('gbk'), 1)
        content = aboveHalf_content + followHalf_content
    ...
    def generateSequenceFile(self):
        ...
        for line in fileLines:# 将ann文件写入anns
            if len(line) <= 2:
                seqFile.write('\n')
                continue
            else:
